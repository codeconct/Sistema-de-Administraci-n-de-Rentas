...
router.get('/rentalcontracts', async (req, res) => {
  try {
    const result = await pool.query("SELECT  c.id, a.name, t.name, c.startdate, c.enddate, c.depositamount FROM rentalcontracts c INNER JOIN apartments a ON c.apartmentid = a.id INNER JOIN tenants t ON c.tenantid = t.id;");
